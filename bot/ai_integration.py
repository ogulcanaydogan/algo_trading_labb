...
class AIOrchestrator:
    ...
    def __init__(self, enable_rl: bool = True, enable_intelligence: bool = True):
        ...
        logger.info(f"AI Orchestrator initialized (RL: {enable_rl}, Intelligence: {enable_intelligence})")
    ...
    async def generate_enhanced_signal(self, symbol: str, ml_signal: Dict, market_data: Dict) -> EnhancedSignal:
        ...
        if self.ai_brain:
            try:
                rl_decision = await self.ai_brain.get_trading_decision(symbol, ml_signal, market_data)
        ...
        final_action, final_confidence = self._combine_signals(
            ml_action, ml_confidence, rl_action, rl_confidence, sentiment_score, fear_greed, ml_meta.get('regime', 'unknown'))
        ...
        reasons = [f"ML: {ml_action} ({ml_confidence:.0%})", f"RL: {rl_action} ({rl_confidence:.0%})"]
        if intelligence_bias != 'neutral':
            reasons.append(f"Sentiment: {intelligence_bias}")

        return EnhancedSignal(
            symbol=symbol, timestamp=datetime.now(), action=final_action, confidence=final_confidence,
            position_size_pct=min(1.0, position_size), stop_loss_pct=stop_loss, take_profit_pct=take_profit,
            trailing_stop=ml_meta.get('volatility', 'medium') == 'high', dca_enabled=final_confidence < 0.7,
            regime=ml_meta.get('regime', 'unknown'), trend=ml_meta.get('trend', 'neutral'),
            volatility=ml_meta.get('volatility', 'medium'), sentiment_score=sentiment_score, fear_greed=fear_greed,
            reason=" | ".join(reasons), ml_action=ml_action, rl_action=rl_action, intelligence_bias=intelligence_bias,
            ml_confidence=ml_confidence, rl_confidence=rl_confidence, sentiment_confidence=0.5 + abs(sentiment_score) * 0.3)

    def _combine_signals(self, ml_action, ml_confidence, rl_action, rl_confidence, sentiment_score, fear_greed, regime):
        ...
        combined = (ml_numeric * ml_confidence * self.weights['ml'] + rl_numeric * rl_confidence * self.weights['rl'] +
                   sentiment_numeric * self.weights['sentiment'] + fear_greed_signal * self.weights['regime'])

        agreement_bonus = 0.15 if ml_action == rl_action and ml_action != 'HOLD' else 0
        conflict_penalty = 0.2 if (ml_numeric > 0 and rl_numeric < 0) or (ml_numeric < 0 and rl_numeric > 0) else 0
        base_confidence = (ml_confidence * self.weights['ml'] + rl_confidence * self.weights['rl']) / (self.weights['ml'] + self.weights['rl'])
    ...
    def record_trade_outcome(self, symbol, action, entry_price, exit_price, pnl, pnl_pct, hold_time_minutes, was_stopped, was_target_hit, features=None):
        if self.ai_brain:
            try:
                self.ai_brain.record_trade_outcome(symbol, action, entry_price, exit_price, pnl, pnl_pct, hold_time_minutes, was_stopped, was_target_hit)
            except Exception as e:
                logger.warning(f"AI Brain record failed: {e}")

        if self.continuous_learner and features is not None:
            try:
                self.continuous_learner.record_prediction(symbol, features, action, 'WIN' if pnl > 0 else 'LOSS', pnl, pnl_pct)
            except Exception as e:
                logger.warning(f"Continuous learner record failed: {e}")

    def get_system_status(self) -> Dict[str, Any]:
        status = {'ai_brain_active': self.ai_brain is not None, 'data_intelligence_active': self.data_intelligence is not None,
                  'continuous_learner_active': self.continuous_learner is not None, 'weights': self.weights}
        if self.ai_brain:
