...
class StrategyPerformance:
    ...
    def expectancy(self) -> float:
        ...
        if self.total_trades == 0:
            return 0.0
        return self.win_rate * self.avg_win_percent - (1 - self.win_rate) * abs(
...
class HistoricalPatternLearner:
    ...
    def _load_patterns(self):
        ...
        try:
            ...
            conn.close()
            logger.info(
                f"Loaded {len(self.profitable_patterns)} profitable patterns, "
...
class TradeOutcomeAnalyzer:
        ...
        if price_history:
            if action == "buy":
                ...
                if optimal_entry_price < entry_price * 0.99:
                    lesson_notes.append(
                        f"Could have entered {((entry_price - optimal_entry_price) / entry_price * 100):.1f}% lower"
                    )
            else:
                optimal_entry_price = max(price_history[: len(price_history) // 4] or [entry_price])
                if optimal_entry_price > entry_price * 1.01:
                    lesson_notes.append(
                        f"Could have entered {((optimal_entry_price - entry_price) / entry_price * 100):.1f}% higher"
                    )
        ...
        if price_history:
            if action == "buy":
                ...
                if optimal_exit_price > exit_price * 1.02:
                    ...
                    lesson_notes.append(
                        f"Exited too early - missed {missed_profit:.1f}% more profit"
                    )
            else:
    ...
    def _save_lesson(
        self, lesson: TradeLesson, entry_grade: str, exit_grade: str, overall_grade: str
    ):
    ...
    def get_insights(self) -> Dict[str, Any]:
        ...
        """)
        best_conditions = [
            {"condition": r[0], "avg_pnl": r[1], "trades": r[2]} for r in cursor.fetchall()
        ]
        ...
        """)
        avoid_conditions = [
            {"condition": r[0], "avg_loss": r[1], "trades": r[2]} for r in cursor.fetchall()
        ]
...
class DailyTargetTracker:
    ...
    def _create_new_day(self, date: str):
        ...
        self.current_day = DailyTarget(
            date=date, target_percent=self.target_percent, max_daily_loss=self.max_daily_loss
        )
        self._save()
    ...
    def record_trade(self, pnl_percent: float) -> Dict[str, Any]:
        ...
        if pnl_percent > 0:
            ...
            self.current_day.current_drawdown = max(
                self.current_day.current_drawdown, abs(pnl_percent)
            )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "win_rate": f"{(self.current_day.wins_today / self.current_day.trades_today * 100):.0f}%"
            if self.current_day.trades_today > 0
            else "N/A",
    ...
    def should_auto_pause(self) -> Tuple[bool, str]:
        ...
        if self.current_day.target_achieved and self.current_day.trades_today >= 5:
            return (
                True,
                f"TARGET ACHIEVED ({self.current_day.current_percent:.2f}%) - Protecting gains",
            )
...
class StrategyGenerator:
    ...
    def __init__(
        self, pattern_learner: HistoricalPatternLearner, data_dir: Path = Path("data/ai_brain")
    ):
    ...
    def evaluate_strategy_for_conditions(
        self, snapshot: MarketSnapshot, signal_confidence: float
        ...
        if best_match and best_score > 0.5:
            ...
            action = (
                "buy"
                if "bull" in best_match.entry_conditions.get("market_condition", "")
                else "sell"
            )
    ...
    def record_strategy_trade(self, strategy_name: str, pnl_percent: float):
        ...
        for strategy in self.strategies:
            if strategy.name == strategy_name:
                ...
                live["win_rate"] = (
                    live["wins"] / live["total_trades"] if live["total_trades"] > 0 else 0
                )
    ...
    def backtest_strategy(self, name: str, historical_data: List[Dict]) -> Dict[str, Any]:
        ...
        for data in historical_data:
            ...
            rsi_state = entry_cond.get("rsi_state", "neutral")
            rsi_match = (
                (rsi_state == "oversold" and snapshot.rsi < 30)
                or (rsi_state == "overbought" and snapshot.rsi > 70)
                or (rsi_state == "neutral" and 30 <= snapshot.rsi <= 70)
            )
        ...
        logger.info(
            f"Backtest {name}: {total_trades} trades, {win_rate:.0%} win rate, {total_pnl:.2f}% total PnL"
        )
        return results
            ...
            if s.name == name:
                ...
                if require_backtest:
                    ...
                    if not backtest.get("profitable", False):
                        return (
                            False,
                            f"Strategy not profitable in backtest (Win rate: {backtest.get('win_rate', 0):.0%}, PnL: {backtest.get('total_pnl', 0):.2f}%)",
                        )
...
class AITradingBrain:
    ...
    def __init__(self, data_dir: Path = Path("data/ai_brain")):
        ...
        self.daily_tracker = DailyTargetTracker(
            target_percent=1.0, max_daily_loss=2.0, data_dir=data_dir
        )

        # Initialize strategy and execution components
        self.strategy_generator = StrategyGenerator(
            pattern_learner=self.pattern_learner, data_dir=data_dir
        )
    ...
    async def analyze_opportunity(
        self, symbol: str, snapshot: MarketSnapshot, signal: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        recommendation = {
            ...
            "take_profit_recommendation": self._recommend_take_profit(pattern_return, daily_status),
        }

        return recommendation

    def _recommend_position_size(self, confidence: float, daily_status: Dict) -> str:
    ...
    def _recommend_stop_loss(self, snapshot: MarketSnapshot, action: str) -> str:
    ...
    def _recommend_take_profit(self, expected_return: float, daily_status: Dict) -> str:
    ...
    def record_trade_result(
        ...
        if price_history:
            ...
            if next_prices:
                next_1h = (
                    (next_prices[min(12, len(next_prices) - 1)] - entry_price) / entry_price * 100
                    if len(next_prices) > 0
                    else 0
                )
                next_4h = (
                    (next_prices[min(48, len(next_prices) - 1)] - entry_price) / entry_price * 100
                    if len(next_prices) > 0
                    else 0
                )
                next_24h = (next_prices[-1] - entry_price) / entry_price * 100

                self.pattern_learner.learn_from_movement(entry_snapshot, next_1h, next_4h, next_24h)
    ...
    def get_brain_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "learning_stats": {
                "patterns_learned": len(self.pattern_learner.profitable_patterns)
                + len(self.pattern_learner.losing_patterns),
    ...
    def get_pattern_prediction(self, snapshot: MarketSnapshot) -> Dict[str, Any]:
