...
class PortfolioManager:
    ...
    def needs_rebalancing(self) -> Tuple[bool, List[str]]:
        ...
        if self.portfolio.rebalance_strategy == RebalanceStrategy.THRESHOLD:
            assets_to_rebalance = [
                asset.symbol for asset in self.portfolio.assets
                if abs(self.get_allocation_drift(asset.symbol)) > self.portfolio.rebalance_threshold_pct
            ]
    ...
    def calculate_portfolio_sharpe(self, returns: List[float], risk_free_rate: float = 0.02) -> float:
    ...
    def get_portfolio_summary(self) -> dict:
        ...
        return {
            ...
            "unrealized_pnl_pct": (self.state.total_unrealized_pnl / self.state.total_capital) * 100,
            "positions": {
                symbol: {
