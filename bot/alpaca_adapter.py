...
class AlpacaAdapter(ExecutionAdapter):
    ...
    def __init__(
        self, api_key: str, api_secret: str, is_paper: bool = True, base_url: Optional[str] = None
    ):
    ...
    async def place_order(self, order: Order) -> OrderResult:
        ...
        try:
            ...
            response = self._session.post(f"{self.base_url}/v2/orders", json=order_data)
            response.raise_for_status()

            result = response.json()
            logger.info(
                f"Alpaca order executed: {result['id']} - {order.side.value} {order.quantity} {symbol}"
            )
    ...
    async def get_market_price(self, symbol: str) -> Optional[float]:
        ...
        try:
            ...
            response = self._session.get(f"{self.data_url}/v2/stocks/{symbol}/quotes/latest")
            response.raise_for_status()
            quote = response.json()
    ...
    async def get_historical_data(
        ...
        try:
            ...
            response = self._session.get(
                ...
                params={"timeframe": alpaca_timeframe, "limit": limit},
            )
            response.raise_for_status()
            data = response.json()
