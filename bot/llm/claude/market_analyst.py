...
Keep response under 150 words."""
    ...
    def generate_daily_market_summary(
        ...
        regime_summary = ", ".join(
            f"{sym}: {reg}" for sym, reg in list(regimes.items())[:5]
        ) if regimes else "Not detected"
...
Keep response under 60 words. No bullet points, just flowing sentences."""
    ...
    def generate_weekly_strategy_review(
        ...
        most_traded = sorted(
            trade_patterns["symbols"].items(), key=lambda x: x[1], reverse=True
        )[:3]
