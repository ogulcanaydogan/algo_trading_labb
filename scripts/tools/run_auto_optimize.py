"""
Continuous Auto Optimization Service

Fetches historical data at specified intervals, optimizes strategy parameters
and writes the best settings to data/strategy_config.json.
The bot reads this file each loop and applies parameters live.
"""
...
def fetch_data(symbol: str, timeframe: str, lookback: int):
    ...
    if use_testnet:
        ...
        if not api_key or not api_secret:
            print("Testnet API keys not found, switching to synthetic data.")
            use_testnet = False
        else:
...
def run_service():
    ...
    while True:
        ...
        try:
            ...
            print(
                "Updated: %s | Sharpe=%.3f PnL%%=%.2f WinRate=%.1f%% MDD%%=%.2f"
                % (
                    out,
                    best.sharpe_ratio,
                    best.total_pnl_pct,
        ...
        if run_once:
            print("AUTO_OPTIMIZE_RUN_ONCE=true - service ran once, exiting.")
            break
