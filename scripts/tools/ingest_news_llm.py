...
Fetches news from RSS feeds, analyzes with LLM, and writes to macro events file
"""
import argparse
import json
...
def fetch_news(feed_urls: List[str], limit: int = 50) -> List[Dict[str, Any]]:
    ...
    print(f"Total {len(all_entries)} news items fetched")
    return all_entries
...
def analyze_with_llm(
    ...
    """
    Analyze news with LLM for each symbol
    """
    macro_events = []
...
def classify_news_basic(
    ...
    """
    Basic keyword-based classification (fallback without LLM)
    """
    ...
    for item in news_items:
        ...
        # Impact (simple keyword matching)
        impact_keywords = {
        ...
        # Affected assets (simple matching)
        affected = []
        for symbol in symbols:
        ...
        if not affected:
            affected = symbols  # Affects all assets
...
def main():
    ...
    print(f"\n{len(feed_urls)} RSS feeds loaded")
    ...
    if args.use_llm:
        ...
        if not llm.health_check():
            print("\nWarning: LLM service is not running!")
            print("Tip: Start with 'ollama serve' or run without --use-llm")
            print("Continuing with VADER...\n")
            macro_events = classify_news_basic(news_items, symbols)
        else:
            print("\nLLM service active, starting analysis...\n")
    ...
    print("\n" + "="*60)
    print(f"{len(macro_events)} events written to: {args.out}")
    print("="*60)
